...
export function LoginForm({
  ...
  const callbackUrl = getPostSignInCallbackUrl({ redirect, priceId, inviteId });
    ...
    <div className="flex min-h-[100dvh] flex-col justify-center bg-gray-50 px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="space-y-6 rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
          ...
          <Link
            href={`${mode === 'signin' ? '/sign-up' : '/sign-in'}${redirect ? `?redirect=${redirect}` : ''}${priceId ? `${redirect ? '&' : '?'}priceId=${priceId}` : ''}${inviteId ? `${redirect || priceId ? '&' : '?'}inviteId=${inviteId}` : ''}`}
