...
import { getCurrentUser, toDisplayUser } from "@/lib/auth/get-current-user";

export default async function EmailAgentLayout({
  children,
}: {
    ...
    <ActiveOrganizationProvider organizationId={organization?.id ?? null}>
      <UserProvider user={toDisplayUser(user)}>
        <AppShell
          ...
          sidebar={<DashboardSidebar sidebarData={sidebarData} />}
