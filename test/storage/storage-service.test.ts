...
describe("storage-service", () => {
  beforeEach(() => {
    ...
    vi.mocked(getCurrentOrganizationEntitlements).mockResolvedValue({
      ...
      planId: "pro",
      planName: "Pro",
      limits: { tasksPerMonth: 1000, teamMembers: 5, storageMb: 5, emailSyncsPerMonth: 50 },
  ...
  it("rejects uploads that exceed the storage limit", async () => {
    vi.mocked(getCurrentOrganizationEntitlements).mockResolvedValue({
      ...
      planId: "pro",
      planName: "Pro",
      limits: { tasksPerMonth: 1000, teamMembers: 5, storageMb: 0.000001, emailSyncsPerMonth: 50 },
      capabilities: ["task.create"],
      activeAddonIds: [],
      billingInterval: "month",
      creditBalance: 1000,
