...
import { hasPlanAccess, resolveOrganizationPlan } from "@/features/billing/plans";
...
describe("resolveOrganizationPlan", () => {
  it("returns the paid plan only while billing access is active", () => {
    ...
    expect(
      resolveOrganizationPlan({ planId: "pro", subscriptionStatus: "trialing" }),
    ).toBe("pro");
  });

  it("downgrades blocked subscription statuses to free access", () => {
    expect(
      resolveOrganizationPlan({ planId: "pro", subscriptionStatus: "past_due" }),
    ).toBe("free");
    expect(
      resolveOrganizationPlan({ planId: "team", subscriptionStatus: "canceled" }),
    ).toBe("free");
